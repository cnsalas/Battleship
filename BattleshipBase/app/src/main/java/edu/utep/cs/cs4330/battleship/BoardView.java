...
public class BoardView extends View {
    ...
    private final int boardHitColor = Color.rgb(254, 131, 103);

    private final Paint boardHitPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    {
        boardHitPaint.setColor(boardHitColor);
    }

    private final int boardMissColor = Color.rgb(252, 254, 103);

    private final Paint boardMissPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    {
        boardMissPaint.setColor(boardMissColor);
    }
    ...
    private void drawPlaces(Canvas canvas) {
        ...
        for(int i = 0; i < boardSize; i++)
        {
            ...
            {
                Place currentAt = board.at(i, j);
                float leftGap = i * placeSize + rectOffset;
                float topGap = j * placeSize + rectOffset;
                float rightGap = leftGap + placeSize - (rectOffset * 2);
                float bottomGap = topGap + placeSize - (rectOffset * 2);
                if(currentAt.isHit() && currentAt.hasShip())
                {
                    canvas.drawRect(leftGap, topGap, rightGap, bottomGap, boardHitPaint);
                }
                else if(currentAt.isHit() && !currentAt.hasShip())
                {
                    canvas.drawRect(leftGap, topGap, rightGap, bottomGap, boardMissPaint);
                }
            }
        }
    }
